...
- **No daemon.** One system crontab line per agent runs `bin/tick` every
...
- **Overlap policy**: previous run of the same workflow still going when the
  next is due — skip (record a skipped occurrence), queue, or run concurrently?
  Agentworks effectively skipped; propose: skip + record.
