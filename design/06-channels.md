...
- **Bindings live in the agent**: `channels.yml` says *which* Telegram bot,
  *which* Planet workspace this agent answers; tokens come from `.env`.
...
- **Inbound resolves to an invocation** through the same runner seam as
  workflows (a message-triggered `reason` with conversation context), so
