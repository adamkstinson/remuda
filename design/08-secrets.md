...
1. **Host** — workflow scripts (`remuda run` / tick) and `remuda console`
   ...
   laptop, `.env` may hold tokens for *self-hosted* MCP.
...
`.env`. Use it while it works; replace the process behind the seam without
touching the gem.
...
Gateway deployment (one shared instance vs per-customer, rotation, `doctor`
...
- **Client gateway operations** — who runs it (operator vs customer),
