...
`adam-server.ts.adamkstinson.com` → `100.64.0.2` (so containers do not
resolve `adam-server` to themselves). `mcp.json` may list both a local
`url` and a `tailscale_url`. On adam-server Remuda uses `url`; everywhere
